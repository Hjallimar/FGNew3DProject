...
public class PointManager : MonoBehaviour {
	...
	void FixedUpdate ()
	{
		...
		for (int i = 0; i < BarcountIndex; i++) 
		{
			float3 diff = GetDiff( points[bars[i].pointIndex1].current,points[bars[i].pointIndex2].current);
			...
			if (points[bars[i].pointIndex1].anchor == false && points[bars[i].pointIndex2].anchor == false) 
			{
				points[bars[i].pointIndex1].AddToCurrent(push); // ADDING NEW DISTANCE/LENGTH 
				points[bars[i].pointIndex2].SubtractFromCurrent(push);
			} 
			else if (points[bars[i].pointIndex1].anchor) 
			{
				points[bars[i].pointIndex2].SubtractFromCurrent(push * 2f);
			} 
			else if (points[bars[i].pointIndex2].anchor) 
			{
				points[bars[i].pointIndex1].AddToCurrent(push * 2);
			}
			...
			if (diff.x/dist * bars[i].oldD.x + diff.y/dist*bars[i].oldD.y + diff.z/dist*bars[i].oldD.z<.99f) 
			{
				...
				bars[i].matrix = Matrix4x4.TRS(new Vector3(
						(points[bars[i].pointIndex1].current.x + points[bars[i].pointIndex2].current.x) * .5f,
						(points[bars[i].pointIndex1].current.y + points[bars[i].pointIndex2].current.y) * .5f,
						(points[bars[i].pointIndex1].current.z + points[bars[i].pointIndex2].current.z) * .5f),
			...
			else 
			{
				...
				matrix.m03 = (points[bars[i].pointIndex1].current.x + points[bars[i].pointIndex2].current.x) * .5f;
				matrix.m13 = (points[bars[i].pointIndex1].current.y + points[bars[i].pointIndex2].current.y) * .5f;
				matrix.m23 = (points[bars[i].pointIndex1].current.z + points[bars[i].pointIndex2].current.z) * .5f;
				bars[i].matrix = matrix;
			}

			if (Mathf.Abs(extraDist) > breakResistance) 
			{
				if (points[bars[i].pointIndex2].neighborCount>1)
				{
					points[bars[i].pointIndex2].neighborCount--;
					Point newPoint = new Point();
					newPoint.CopyFrom(points[bars[i].pointIndex2]);
					newPoint.neighborCount = 1;
					points[pointCount].CopyFrom(newPoint);
					bars[i].CopyToPoint2(newPoint);
					pointCount++;
				} 
				else if (points[bars[i].pointIndex1].neighborCount>1) 
				{
					points[bars[i].pointIndex1].neighborCount--;
					Point newPoint = new Point();
					newPoint.CopyFrom(points[bars[i].pointIndex1]);
					newPoint.neighborCount = 1;
					points[pointCount].CopyFrom(newPoint);
